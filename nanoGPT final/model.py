...
class CausalSelfAttentionGQA(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.n_head = config.n_head
        self.group_size = config.n_headgroup
        self.n_kv_head = config.n_head // config.n_headgroup  # number of shared key/value heads
        self.head_dim = config.n_embd // config.n_head

        # project queries to all heads, but keys/values only to reduced n_kv_head heads
        ...
        # causal mask to keep attention left-only
        self.register_buffer("bias", torch.tril(torch.ones(config.block_size, config.block_size))
                                        .view(1, 1, config.block_size, config.block_size))
    ...
    def forward(self, x):
        ...
        qkv = self.c_attn(x)
        q_split = self.n_head * self.head_dim
        kv_split = self.n_kv_head * self.head_dim
        q, k, v  = qkv.split([q_split, kv_split, kv_split], dim=2)
        ...
        # share k/v across groups of queries
        k = k.repeat_interleave(self.group_size, dim=1)                   # (B, n_head, T, hd)
        v = v.repeat_interleave(self.group_size, dim=1)                   # (B, n_head, T, hd)
